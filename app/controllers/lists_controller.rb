...
# for J+S Database
  ...
  def email
    ...
    people = Person.find_by_sql(["select firstname, lastname, email, bill_email, course_id from people inner join courses_people on people.id = courses_people.person_id where (people.leave_date is null or people.leave_date > date('now')) and course_id in (?)", courses])
